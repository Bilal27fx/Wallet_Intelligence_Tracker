...
class SimpleFIFOAnalyzer:
    ...
    def get_wallet_transactions(self, wallet_address: str) -> Dict[str, List[Dict]]:
        ...
        Utilise les prix hérités lors de migrations de wallets si disponibles
        """
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for tx in transactions:
                    ...
                    # NOUVEAU: Utiliser le prix hérité en priorité si disponible (migration de wallet)
                    effective_price = float(inherited_price) if inherited_price else float(price)
                    price_val = effective_price
                    value_val = float(value)

                    # Recalculer la valeur avec le prix hérité si applicable
                    ...
                    tokens[token_key]['txs'].append({
                        ...
                        'direction': direction,
                        'quantity': float(qty),
                        'price': price_val,  # Prix effectif (hérité ou réel)
                        'value': value_val,
    ...
    def calculate_fifo_metrics(self, token_data: Dict) -> Optional[Dict]:
        ...
        # Les quantités OUT peuvent être négatives dans la DB, utiliser abs()
        ...
        # Les valeurs OUT peuvent être négatives dans la DB, utiliser abs()
