...
# === Chargement de l'API KEY avec rotation ===
...
class APIKeyManager:
    ...
    def rotate_key(self):
        if len(self.keys) <= 1:
            print("⚠️ Une seule clé API disponible")
            return False
...
class SimpleWalletHistoryExtractor:
    ...
    def update_headers(self):
        """Met à jour les headers avec la clé API actuelle"""
            ...
            try:
                ...
                if response.status_code == 429:
                    ...
                    if api_manager.rotate_key():
                        ...
                        time.sleep(5)
                        # Retry après rotation
                        continue  # Reprendre la même page
                    else:
                        print("    ❌ Toutes les clés API ont atteint leur quota")
                        return None
