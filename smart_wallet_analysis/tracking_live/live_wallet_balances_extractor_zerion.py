...
ROOT = Path(__file__).parent.parent.parent  # Remonter de smart_wallet_analysis/tracking_live/ vers la racine
...
def get_existing_wallet_tokens(wallet_address, filter_smart_wallets=True):
    ...
    Args:
        ...
        filter_smart_wallets: Si True, ne traite que les smart wallets (défaut)
                             Si False, traite tous les wallets (pour re-scoring)
    """
    try:
        conn = sqlite3.connect(DB_PATH)
        ...
        if filter_smart_wallets:
            ...
            # Mode re-scoring: tous les wallets
            query = """
                SELECT t.symbol, t.current_amount, t.current_usd_value, t.contract_address,
                       t.chain, t.fungible_id, t.updated_at
                FROM tokens t
...
def log_token_change_to_db(change_data, change_type, cursor=None):
    """Enregistre un changement de token dans l'historique des transactions

    Args:
        change_data: Dictionnaire contenant les données du changement
        change_type: Type de changement ("new_token", "accumulation", "reduction", "exit")
        cursor: Curseur SQLite existant (optionnel, si fourni évite d'ouvrir une nouvelle connexion)
    """
    try:
        # Si un curseur est fourni, l'utiliser (évite database is locked)
        # Sinon créer une nouvelle connexion (backward compatibility)
        ...
        total_usd = change_data.get("new_usd_value", change_data.get("usd_value", 0))
        price_per_token = (total_usd / quantity) if quantity > 0 else 0
        ...
        cursor.execute("""
            INSERT INTO transaction_history (
                ...
                direction, quantity, price_per_token, total_value_usd
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
        ...
        # Commit et fermer seulement si on possède la connexion
        if owns_connection:
            conn.commit()
            conn.close()
        ...
        if owns_connection and 'conn' in locals():
            conn.close()
        return False
...
def detect_position_changes_sql(wallet_address, current_tokens_data, session_id):
    ...
    try:
        ...
        for symbol in new_symbols:
            ...
            # Enregistrer aussi dans transaction_history pour le FIFO
            log_token_change_to_db(change, "new_token", cursor)
        ...
        for symbol in common_symbols:
            ...
            if (abs(amount_change) / max(previous["amount"], 0.001) > change_pct_threshold and 
                ...
                change = {
                    ...
                    "fungible_id": current["fungible_id"]
                }
                ...
                # Enregistrer aussi dans transaction_history pour le FIFO
                if change_type == "ACCUMULATION":
                    log_token_change_to_db(change, "accumulation", cursor)
                else:
                    log_token_change_to_db(change, "reduction", cursor)
        ...
        for symbol in exited_symbols:
            ...
            change = {
                ...
                "wallet_address": wallet_address,
                "change_type": "EXIT",
                "contract_address": previous.get("contract_address", ""),
                "fungible_id": previous.get("fungible_id", "")
            }
            ...
            # Enregistrer aussi dans transaction_history pour le FIFO
            log_token_change_to_db(change, "exit", cursor)
