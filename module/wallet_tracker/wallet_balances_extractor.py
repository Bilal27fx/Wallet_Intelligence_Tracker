...
MIN_WALLET_VALUE_USD = 200000  # 200k USD minimum
MAX_WALLET_VALUE_USD = 50000000  # 50M USD maximum
MIN_TOKENS_PER_WALLET = 4  # Minimum 3 tokens par wallet
...
def get_token_balances_zerion(address):
    ...
    try:
        ...
        # Rejeter le wallet s'il a moins de 3 tokens valides (hors exclus)
        if len(valid_positions) < 3:
            print(f"    🚫 Wallet rejeté: {len(valid_positions)} tokens valides < 3 (minimum requis)")
            return pd.DataFrame()
