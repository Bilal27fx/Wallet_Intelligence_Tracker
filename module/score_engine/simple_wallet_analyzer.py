...
def analyze_wallet_simple(wallet_address):
    ...
    # Analyser chaque palier
    for tier in range(3000, 13000, 1000):
        ...
        # Classification
        gagnants = sum(1 for t in tier_tokens if t[2] >= 80)
        perdants = sum(1 for t in tier_tokens if t[2] < 0)
...
def save_wallet_profile(wallet_address, tier_results):
    ...
    try:
        ...
        # Ajouter les données pour chaque palier (3k à 12k)
        for tier_num in range(3, 13):
